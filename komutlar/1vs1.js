		...
		try {
			...
			while (userHP > 0 && oppoHP > 0) { // eslint-disable-line no-unmodified-loop-condition
				...
				if (choice === 'saldır') {
					...
					await message.channel.send(`${user}, çöp kovasını kafasına geçirerek korundu!`);
					guard = true;
					reset(false);
					...
					if (!miss) {
						...
						await message.channel.send(`${user}, thor un özel güçlerini kullanarak **${damage}** hasar vurdun!!`);
						dealDamage(damage);
					} else {
						await message.channel.send(`${user}, thor un özel güç enerjisini yeterli miktarda toplayamadığın için ulta güç kullanamadın!`);
					}
					reset();
				} else if (choice === 'kaç') {
					await message.channel.send(`${user}, kaçtı! Oyun Bitti!`);
					forfeit();
					break;
				} else {
